...
export let makeEffect = <T,U,V,W>(pair: ActionPair<T>, completeAction: ActionCtor<U>, mapper: (v: T) => { obs: Observable<U>, complete?: V, fail?: W }, opts: {
  ...
  let payload$ = initialized$
      ...
      .map(R.prop('payload'));
  let lambda = (v: T) => {
    let { obs: Observable<U>, complete?: V, fail?: W } = mapper(v);
    let defaulted$ = !R.isNil(fallback) ? obs.catch(() => Observable.of(<U> fallback)) : obs;
    let created$ = defaulted$.map((d: U) => new completeAction(complete || d));
    let caught$ = failAction /*instanceof Object*/ ? created$.catch((error: Error) => Observable.of(new (<ActionCtor<Error>>failAction)(fail || error))) : created$;
    return caught$;
  };
